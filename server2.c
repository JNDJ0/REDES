 ...
 */
int P2PConnect(char* ip, int peer_port, int my_socket) {
    struct sockaddr_in peer_addr;
    int peer_socket = socket(AF_INET, SOCK_STREAM, 0);
    if (peer_socket < 0) {
        error("P2P: ERROR opening peer socket");
    }
    ...
    if (inet_pton(AF_INET, ip, &peer_addr.sin_addr) <= 0) {
        close(peer_socket);
        error("P2P: ERROR invalid peer target IP address");
    }
    peer_addr.sin_port = htons(peer_port);

    // Tentativa de conexão com o outro peer
    if (connect(peer_socket, (struct sockaddr *)&peer_addr, sizeof(peer_addr)) == 0) {
        // Inicia troca de mensagens
        SendMessage(REQ_CONNPEER, "", peer_socket);
        ...
        fd_set read_fds;
        int max_fd = (my_socket > STDIN_FILENO) ? my_socket : STDIN_FILENO;
        while(1){
            FD_ZERO(&read_fds);
            ...
            if (FD_ISSET(my_socket, &read_fds)) {
                ...
                int accepted_socket = accept(my_socket, (struct sockaddr *)&incoming_addr, &addrlen);
                if (accepted_socket < 0) {
                    error("P2P: ERROR on accept");
                }
        
                // Inicia troca de mensagens
                message msg = ReceiveRawMessage(accepted_socket);
                if (msg.type == REQ_CONNPEER) {
                    GenerateRandomID(peer_id);
                    printf("Peer %s connected\n", peer_id);
                    SendMessage(RES_CONNPEER, peer_id, accepted_socket);
                    
                    msg = ReceiveRawMessage(accepted_socket);
                    if (msg.type == RES_CONNPEER) {
                        connected_peers++;
                        strncpy(my_id, msg.payload, ID_LEN);
                        my_id[ID_LEN] = '\0';
...
int P2PListener(char* ip, int port){
    ...
    if (sock < 0) {
        error("P2P: ERROR opening socket");
    }
    ...
    if (bind(sock, (struct sockaddr *)&peer_addr, sizeof(peer_addr)) < 0) {
        error("P2P: ERROR on binding socket"); 
    }
...
int main(int argc, char **argv) {
    ...
    char buffer[256];
    struct sockaddr_in peer_addr;
    int my_port, peer_port, sensor_port, n, sensor_listener_socket, peer_listener_socket, peer_socket, max_fd;
    connected_peers = 0;
    connected_sensors = 0;
    ...
    ip = argv[1];
    my_port = atoi(argv[2]);
    peer_port = atoi(argv[2]) == P2P_FIRST_PORT ? P2P_SECOND_PORT : P2P_FIRST_PORT;
    sensor_port = atoi(argv[3]);
    int role = sensor_port == SL_CLIENT_LISTEN_PORT_DEFAULT ? 1 : 0;

    // Conectando aos sockets do peer e do sensor
    peer_listener_socket = P2PListener(ip, my_port);
    if (peer_listener_socket < 0) return 1;
    sensor_listener_socket = SensorConnect(ip, sensor_port);
    peer_socket = P2PConnect(ip, peer_port, peer_listener_socket);
    ...
    while (1) {
        ...
        FD_SET(peer_listener_socket, &read_fds);
        // Aguardando entradas no terminal 
        FD_SET(STDIN_FILENO, &read_fds);
        // Aguardando mensagens dos sensores
        for (int i = 0; i < connected_sensors; i++) {
            FD_SET(sensors[i].socket_fd, &read_fds);
        }
        // Aguardando mensagens do peer
        ...
        if (peer_listener_socket > max_fd) max_fd = peer_listener_socket;
        if (STDIN_FILENO > max_fd) max_fd = STDIN_FILENO;
        for (int i = 0; i < connected_sensors; i++) {
        ...
        if (FD_ISSET(peer_listener_socket, &read_fds)) {
            printf("%d\n", connected_peers);
            // Aceitando a conexão feita
            // peer_socket = P2PConnect(ip, peer_port, peer_listener_socket);
            // if (peer_socket < 0) return 1;
